  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at 88% 4%, rgba(47,107,255,.19), transparent 30%),
        radial-gradient(circle at 2% 98%, rgba(167,179,196,.10), transparent 34%),
        var(--paper);
    ...
    .topline { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; margin: 4px 2px 14px; }
    ...
    .card { border:1px solid var(--line); border-radius:22px; background:var(--card); box-shadow:var(--shadow); backdrop-filter:blur(16px); }
    ...
    .tabs { display:flex; gap:4px; margin:14px 0 9px; padding:4px; border:1px solid var(--line); border-radius:14px; overflow:auto; background:var(--card); }
    ...
    .btn.ghost { padding:5px 8px; font-size:11px; color:var(--muted); }
    ...
    .remove-ingredient { width:42px; height:44px; border:1px solid rgba(47,107,255,.34); border-radius:12px; color:var(--rose); background:rgba(47,107,255,.10); font-size:23px; font-weight:900; cursor:pointer; touch-action:manipulation; }
    .add-ingredient { width:100%; margin-top:10px; border-style:dashed; color:var(--lime); }
    .ingredient-breakdown { display:grid; gap:7px; margin-top:14px; padding-top:13px; border-top:1px solid var(--line); }
    .ingredient-line { display:flex; justify-content:space-between; gap:12px; color:var(--muted); }
    .ingredient-line > span { flex:1; min-width:0; }
    .ingredient-line strong { color:var(--ink); font-weight:900; }
    .ingredient-line .remove-ingredient { flex:0 0 38px; width:38px; height:38px; }
    ...
    .day-card { margin-top:14px; padding:18px; overflow:hidden; }
    ...
    @media (max-width:400px) {
      ...
      .topline > .pill { display:inline-block; margin-top:10px; }
    ...
    (() => {
      ...
      function dayTotalsCard(day, options = {}) {
        ...
        const comparison = options.projected && options.current
          ? '<div class="day-comparison">Currently saved: <strong>' +
            ...
            num(options.mealCalories) + ' cal</strong>. Nothing changes until you confirm.</div>'
          : "";
        return '<section class="card day-card"><div class="day-head"><div><div class="eyebrow">' +
          (options.projected ? "Day total if saved" : "Day total") +
      ...
      function ingredientEditor(items) {
        return '<div class="ingredient-stack">' + items.map((item, index) =>
          ...
          esc(item.name) + ' calories"></label><button class="remove-ingredient" data-remove-item="' +
      ...
      function renderPreview(data) {
        ...
        root.innerHTML =
          '<div class="topline"><div><div class="eyebrow">Meal preview</div><h1>' +
          esc(meal.name || "Meal estimate") + '</h1></div><span class="pill">' +
          Math.round(Number(meal.confidence || 0) * 100) + '% confidence</span></div>' +
          '<section class="card preview"><div class="preview-total"><div><div class="eyebrow">Meal total</div>' +
          ...
          '</section><div id="projection">' + dayTotalsCard(data.projectedDay, {
            projected: true,
            current: data.day,
            mealCalories: meal.totals?.calories
          }) + '</div>' +
          '<div id="preview-status"></div><div class="actions"><button id="confirm" class="btn primary">Save meal</button>' +
          '<button id="adjust" class="btn">Change in chat</button></div>';
      ...
      function renderSaved(data) {
        ...
        const day = data.day || {};
        const items = meal.items || [];
        root.innerHTML = '<div class="topline"><div><h1>' +
          esc(meal.name || "Meal") + '</h1></div><span class="pill">' + esc(meal.localDate || "") +
          '</span></div><section class="card preview">' +
          '<div class="preview-total"><div><div class="metric-big">' + num(meal.totals?.calories) +
          '</div><div class="metric-sub">calories</div></div><div class="row-value">' +
          num(meal.totals?.proteinG) + 'g protein<br><span class="row-meta">' +
          num(meal.totals?.carbsG) + 'g carbs · ' + num(meal.totals?.fatG) +
        ...
        document.getElementById("open-dashboard").onclick = async () => {
          const result = await callTool("render_dashboard", { date: meal.localDate });
          if (result?.structuredContent) render(result.structuredContent);
        };
      }
      function renderDaySummary(data) {
        const day = data.day || {};
        root.innerHTML = '<div class="topline"><div><div class="eyebrow">Nutrition tracker</div><h1>Daily totals</h1></div>' +
      ...
      function renderDashboard(data) {
        ...
        root.innerHTML =
          '<div class="topline"><div><div class="eyebrow">Nutrition tracker</div><h1>Daily totals</h1></div><span class="pill">' +
          esc(today.date || "") + '</span></div>' +
          '<section class="card hero"><div class="hero-main"><div class="eyebrow">Calories</div><div class="metric-big">' +
          num(totals.calories) + '</div><div class="metric-sub">of ' + num(goals.calorieGoal) +
          ' goal</div><div class="progress"><i style="width:' + pct(totals.calories, goals.calorieGoal) +
          '%"></i></div></div><div class="hero-side"><div class="eyebrow">Remaining</div><div><div class="remaining">' +
          (remaining.calories == null ? "Set a goal" : num(remaining.calories) + " cal") +
          '</div><div class="metric-sub">' + num(totals.proteinG) + 'g protein on this day</div></div></div></section>' +
          '<div class="macros"><div class="card macro"><div class="eyebrow">Protein</div><strong>' +
          num(totals.proteinG) + 'g</strong><div class="row-meta">' + num(remaining.proteinG) +
          'g left</div></div><div class="card macro"><div class="eyebrow">Carbs</div><strong>' +
          num(totals.carbsG) + 'g</strong><div class="row-meta">' + num(remaining.carbsG) +
          'g left</div></div><div class="card macro"><div class="eyebrow">Fat</div><strong>' +
          num(totals.fatG) + 'g</strong><div class="row-meta">' + num(remaining.fatG) +
          'g left</div></div><div class="card macro"><div class="eyebrow">Fiber</div><strong>' +
          num(totals.fiberG) + 'g</strong><div class="row-meta">' + num(remaining.fiberG) +
          'g left</div></div></div>' +
          ...
          '</div></div>').join("") + '</div></div><div class="macros"><div class="card macro"><div class="eyebrow">Avg calories</div><strong>' +
          num(data.week?.averages?.calories) + '</strong></div><div class="card macro"><div class="eyebrow">Protein consistency</div><strong>' +
          num(data.week?.proteinConsistencyPct) + '%</strong></div></div></section>' +
          '<section id="panel-weight" class="panel"><div class="card">' +
          ...
          '</div></section>' +
          '<section id="panel-saved" class="panel"><div class="card list">' +
          (saved.length ? saved.map((meal) => '<div class="row"><div><div class="row-title">' + esc(meal.name) +
            ...
            ' cal</div><button class="btn ghost" data-log-saved="' + esc(meal.id) + '">Log now</button></div></div>').join("")
            : '<div class="empty">No saved meals yet.</div>') + '</div></section>' +
          '<section id="panel-review" class="panel"><div class="card list">' +
